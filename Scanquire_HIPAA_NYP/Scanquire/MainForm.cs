...
namespace Scanquire
{
    public partial class MainForm : Form
    {
        ...
        public MainForm()
        {
            ...
            tsRunAutoQA.Visible = false;
            tsRunAutoQA.CheckState = CheckState.Unchecked;
        ...
        protected void MemoryUsed(string message)
        {
            try
            {
                ...
                var memoryMD = currentProc.PrivateMemorySize64 / (1024 * 1024);
                EDL.TraceLogger.TraceLoggerInstance.TraceInformation(string.Format("{0} {1} bytes", message, memoryUsed));
                EDL.TraceLogger.TraceLoggerInstance.TraceInformation(string.Format("{0} {1} mb", message, memoryMD));
                currentProc.Dispose();
            }
            catch (Exception ex)
            {
        ...
        private void LoginUser()
        {
            ...
            if (!(Public.ScanQuireUsers.ScanQuireUser.ScanQuireInstance.Login()))
                this.Close();
            this.Enabled = false;
          
            this.Visible = true;
           EDL.TraceLogger.TraceLoggerInstance.UserName = Public.ScanQuireUsers.ScanQuireUser.ScanQuireInstance.ScanQuireUserLoggedIn.Trim();
      ...
      //  }
        ...
        {
            EDL.TraceLogger.TraceLoggerInstance.TraceWarning("User Closing scanquire");
            this.Close();
        }

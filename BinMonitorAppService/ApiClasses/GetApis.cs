...
#pragma warning disable 1998
        ...
        public async Task<IDictionary<string, BinRegProcessModel>> ApiProcessingStarted(string apiUrl, string sp, bool toList, ILog log)
        {
            IDictionary<string, BinRegProcessModel> binRegPro = null;
            try
            {
                        ...
                        if (toList)
                        {
                            ...
                            foreach (string id in binId)
                            {
                                ...
                                if (array.Count > 0)
                                {
                                    ...
                                    if (binRegProList.TryGetValue(array[0], out BinRegProcessModel model))
                                    {
                                        //  if(model.ProcessStartAt.Year > 2000)
                                        binRegPro.Add(id, model);
                                    }
        ...
        public async Task<IDictionary<int, TransferReportModel>> TransferRep(string apiUrl, string controller, DateTime sDate, DateTime eDate, string transType, ILog log)
        {
            IDictionary<int, TransferReportModel> retTrans = null;
            try
            {


                //  log.LogInformation($"Method ApiActiveBinsByBatchId weburl: {apiUrl} controller: {sp} to list: {convertToList.ToString()}");
                log.LogInformation($"Method TransferRep weburl: {apiUrl} controller: {controller} for sdate {sDate} edate {eDate} transfer type {transType}");
                using (var client = new HttpClient())
                {
                    client.BaseAddress = new Uri($"{apiUrl}{controller}");
                    string dateS = sDate.ToString("MM-dd-yyyy");
                    string dateE = eDate.ToString("MM-dd-yyyy");
                    var responseTask = client.GetAsync($"{dateS}/{dateE}/{transType}");
                    responseTask.Wait();
                    var results = responseTask.Result;
                    log.LogInformation($"Method TransferRep weburl: {apiUrl} controller: {controller} for sdate {sDate} edate {eDate} transfer type {transType} results stats code: {results.StatusCode}");
                    if (results.IsSuccessStatusCode)
                    {
                        var readTask1 = results.Content.ReadAsStringAsync();
                        readTask1.Wait();

                        var readTask = results.Content.ReadAsAsync<TransferReportModel[]>();
                        readTask.Wait();
                        retTrans = readTask.Result.ToDictionary(p => p.ID);
                        ...
                        //  throw new Exception($"Method ApiActiveBinsByBatchId weburl: {apiUrl} controller: {sp} to list: {convertToList.ToString()} results stats code: {results.StatusCode}");
                        throw new Exception($"Method TransferRep weburl: {apiUrl} controller: {controller} for sdate {sDate} edate {eDate} transfer type {transType}  results stats code: {results.StatusCode}");
                }
            }
            catch (HttpRequestException httpex)
            {
                  log.LogError($"HttpRequestException Method TransferRep weburl: {apiUrl} controller: {controller} for sdate {sDate} edate {eDate} transfer type {transType} {httpex.Message}");
                throw new Exception($"HttpRequestException Method TransferRep weburl: {apiUrl} controller: {controller} for sdate {sDate} edate {eDate} transfer type {transType}   {httpex.Message}");
            }

            catch (ArgumentNullException ag)
            {
                   log.LogError($"ArgumentNullException Method TransferRep weburl: {apiUrl} controller: {controller} for sdate {sDate} edate {eDate} transfer type {transType} results stats code:  {ag.Message}");
                throw new Exception($"ArgumentNullException Method TransferRep weburl: {apiUrl} controller: {controller} for sdate {sDate} edate {eDate} transfer type {transType} results stats code:  {ag.Message}");
            }
            catch (Exception ex)
            {
                  log.LogError($"Exception Method TransferRep weburl: {apiUrl} controller: {controller} for sdate {sDate} edate {eDate} transfer type {transType} results stats code:  {ex.Message}");
                throw new Exception($"Exception Method TransferRep weburl: {apiUrl} controller: {controller} for sdate {sDate} edate {eDate} transfer type {transType}results stats code: {ex.Message}");
            }
            return retTrans;
        }
    }

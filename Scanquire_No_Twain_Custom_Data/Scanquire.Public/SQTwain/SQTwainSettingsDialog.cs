...
namespace Scanquire.Public
{
	...
	public class SQTwainSettingsDialog : TableLayoutPanelInputDialog
    {
        ...
        protected void AddNewSettingXmlFile(string selSetting)
        {

            EdocsUSA.Utilities.SettingsManager.UpDatedCustomDataFile = true;
            if(!(string.IsNullOrWhiteSpace(selSetting)))
            Twain.GetCustomDSData();
            ...
            //   string sourceDirPath = Path.Combine(SQTwainSettings.Instance.DirectoryPath, $"{SelectedScanner}\\CustomDataSettings");

            //byte[] customDSData = SQTwainSettings.Instance[SelectedScanner][SelectedSetting].CustomDSData;
            //string s = Encoding.Default.GetString(customDSData);
            byte[] customDSData = CopySettingsFiles(settingName);
        ...
        protected byte[] CopySettingsFiles(string selectedName)
        {
            string sourceDirPath = Path.Combine(SQTwainSettings.Instance.DirectoryPath, $"{SelectedScanner}\\CustomDataSettings");
            Edocs_Utilities.EdocsUtilitiesInstance.CreateDir(sourceDirPath);
            string settFName = string.Empty;
            foreach (var sourceFileName in Edocs_Utilities.EdocsUtilitiesInstance.GetFiles(EdocsUSA.Utilities.SettingsManager.TwainSettingsXmlFolder, "*.*"))
            {
                string sFileName =$"{Edocs_Utilities.EdocsUtilitiesInstance.GetFileName(sourceFileName)}_{selectedName}{Path.GetExtension(sourceFileName)}";
                string destFile = Path.Combine(sourceDirPath, sFileName);
                
                settFName += $"{destFile},{sourceFileName};";
                //Edocs_Utilities.EdocsUtilitiesInstance.CopyFile($"{(char)(34)}{sourceFileName}{(char)(34)}", destFile, true, false);
                Edocs_Utilities.EdocsUtilitiesInstance.CopyFile($"{sourceFileName}", destFile, true, false);
            }
            byte[] customDSData = Encoding.Default.GetBytes(settFName);
            return customDSData;
        }
        protected void AddNewSetting()
        {
            ...
            Twain.SetActiveDataSource(SelectedScanner);
            if (SelectedScanner.ToUpper().StartsWith(EdocsUSA.Utilities.SettingsManager.TwainSettingsUseXmlFile.ToUpper()))
            {
                AddNewSettingXmlFile(SelectedSetting);

    ...
    public partial class MainForm : Form
    {
        ...
        private async Task Acquire(ISQArchiver archiver, SQAcquireIntent intent, SQAcquireSource source)
        {
            ...
            if (tsImageRecovery.Checked)
            {
                ...
                Edocs_Utilities.EdocsUtilitiesInstance.BakupOrgImages = false;
            }

            // Edocs_Utilities.EdocsUtilitiesInstance.BackUpFolderScanTime = DateTime.Now.ToString("yyyyy_MM_dd_HH_ss");
            ...
            if (Edocs_Utilities.EdocsUtilitiesInstance.BakupOrgImages)
            {
                if (ImageListViewer.ItemCount == 0)
                {
                    //   Edocs_Utilities.EdocsUtilitiesInstance.RecoverImagesOrder = new List<string>();
                    Edocs_Utilities.EdocsUtilitiesInstance.BackUpFolderScanTime = DateTime.Now.ToString("yyyy_MM_dd_HH_ss");
                }
            }
        ...
        protected async Task Save(ISQArchiver archiver)
        {
            ...
            try
            {
                ...
                {
                    if (Edocs_Utilities.EdocsUtilitiesInstance.BackUpImagesInput(SQFilesystemConnector.BackUpImagesInput))
                        Array.Clear(SQFilesystemConnector.BackUpImagesInput,0, SQFilesystemConnector.BackUpImagesInput.Length);
                }
            }
            catch (OperationCanceledException oe)
            {
        ...
        private void AutoQa(IEnumerable<Scanquire.Public.SQImage> images, CancellationToken cToken)
        {
        ...
        private void AutoQA(IEnumerable<Scanquire.Public.SQImage> images, bool delBlankImages)
        {
            CancellationTokenSource cTokenSource = new CancellationTokenSource();
            CancellationToken cToken = cTokenSource.Token;
            Progress<ProgressEventArgs> progress = new Progress<ProgressEventArgs>();
        ...
        private void LoginUser()
        {
            ...
            this.Enabled = false;
          bool userCancled=  Public.ScanQuireUsers.ScanQuireUser.ScanQuireInstance.Login();
            if (userCancled)
                Close();
            this.Visible = true;
        ...
        private async void RestoreImages()
        {
            // using (StreamReader sr = new StreamReader(@"D:\Archives\ScanQuireSavedImages\e-Docs USA\Scanquire\02_15_2023\RestorImageFiles_02023_02_15_08_01.txt"))
            //{
            //  string str = string.Empty;
            //  while ((str = sr.ReadLineAsync().ConfigureAwait(false).GetAwaiter().GetResult()) != null)
            // {
            ...
            try
            {
                ...
                Edocs_Utilities.EdocsUtilitiesInstance.DecriptImage = true;
                Edocs_Utilities.EdocsUtilitiesInstance.BakupOrgImages = false;
                //string getFiles = File.ReadAllText(@"D:\Archives\ScanQuireSavedImages\e-Docs USA\Scanquire\02_15_2023\RestorImageFiles_02023_02_15_09_26.txt");
                //if (getFiles.EndsWith(","))
                //    getFiles = getFiles.Remove(getFiles.Length - 1, 1);
                RecoveryImages recoveryImages = new RecoveryImages();
                recoveryImages.ShowDialog();
                if ((recoveryImages.ImagesFiles != null) && (recoveryImages.ImagesFiles.Length > 0))
                {
                    SQFilesystemConnector.RestorImagesPath = recoveryImages.ImagesFiles;
            ...
            {
                MessageBox.Show($"Error Getting Images {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            finally
            {
                tsImageRecovery.Checked = false;
                tsImageRecovery.CheckState = CheckState.Unchecked;
                Edocs_Utilities.EdocsUtilitiesInstance.DecriptImage = false;
                SQFilesystemConnector.RestorImagesPath = null;
                Edocs_Utilities.EdocsUtilitiesInstance.BakupOrgImages = true;
            }
        }
        private async Task OpenFiles()
        {
            if (ImageListViewer.ItemCount > 0)
            {
                if (MessageBox.Show("Append Restored Images", "Append images", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
                { 
                    Edocs_Utilities.EdocsUtilitiesInstance.BackUpFolderScanTime = DateTime.Now.ToString("yyyy_MM_dd_HH_ss"); 
                
                    ImageListViewer.ClearAll(true);
                }
            }
            if (ImageListViewer.ItemCount > 0)
                await Acquire(LocalArchiver, SQAcquireIntent.Append, SQAcquireSource.File);
            ...
            try
            {
                ...
                for (int i = 0; i < item.Count(); i++)
                {
                    if (item[i].Selected)
                    {
                        ...
                        //    //    Image img = Image.FromFile(Path.Combine(imageFolder, $"PrintImage_{imagesCount.ToString()}.png"));
                        //     //   printPreview.Document = img;
                        //        PrintDocument.Print();
                        //    }
                        //    else
                        //        return;

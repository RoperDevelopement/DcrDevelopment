    ...
    public class AutoQaCheckBlankImages
    {
        ...
        public void AutoQa(SQImage image, IProgress<ProgressEventArgs> progress, int totalQa)
        {
            ...
            if(IsImageBlank(image))
           {
            ...
            try
            {
               // if (image.WorkingCopy == null)
               // {
               //     //   Bitmap bMap = new Bitmap(image.LatestRevision.GetOriginalImageBitmap());
               //     image.WorkingCopy = new FreeImageBitmap(image.LatestRevision.GetOriginalImageBitmap());
               //// MessageBox.Show($"{image.WorkingCopy}");
               // }
                using (Bitmap bitmap = (Bitmap)image.WorkingCopy)
                {

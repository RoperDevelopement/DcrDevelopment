	...
	public class SQFileReader_Pdf : SQFileReaderBase
	{
        ...
        public virtual IEnumerable<Task<SQImage>> Read(SQFile file, IEnumerable<string> passwords, IProgress<ProgressEventArgs> progress, CancellationToken cToken)
        {
            ...
            try
            {
                ...
                //added since when have pdf files created by somone else it will not read the pdf files
                if (!(pdf.GetDocumentMetadata().ToLower().Contains("e-doc")))
                {
                    ETL.TraceLoggerInstance.TraceInformation("Pdf not created by e-Docs");
                    for (int currentPageIndex = 1; currentPageIndex <= pageCount; currentPageIndex++)
                    {
                        ...
                        // SaveToFile(pdf,300, currentPageIndex, 1,  @"L:\pdfimage\t.PNG").ConfigureAwait(false).GetAwaiter().GetResult();

                        System.Windows.Forms.Application.DoEvents();
                        // pdf.RenderDocumentToFile(300, currentPageIndex, 1, 0, @"L:\pdfimage\t.PNG");

                        yield return Task.Factory.StartNew<SQImage>(() =>

                        {
                            // byte[] imageData = System.IO.File.ReadAllBytes($@"l:\pdfimage\t{currentPageIndex}.png");//pdf.GetImageListItemDataToString(imageListID, imageIndex, 0);
                            //using (Bitmap bitmap = new Bitmap($@"l:\pdfimage\t{currentPageIndex}.png"))
                            // byte[] imgbyte = pdf.RenderPageToString(300, currentPageIndex, 0);
                            using (var ms = new MemoryStream(pdf.RenderPageToString(300, currentPageIndex, 0)))
                            {
                ...
                {
                    ETL.TraceLoggerInstance.TraceInformation("Pdf  created by e-Docs");

...
namespace Edocs.Libaray.AzureCloud.Upload.Batches
{
    public static class UploadUtilities
    {
        ...
        public static async Task<string> SaveTffLocalFile(string batchDir, string batchID, string uploadFileName, string saveFileName)
        {
            uploadFileName = uploadFileName.Replace('/', '\\');
            uploadFileName = $"{Edocs_Utilities.EdocsUtilitiesInstance.CheckDirectoryPath(uploadFileName)}\\{batchID}\\";
            Edocs_Utilities.EdocsUtilitiesInstance.CreateDir(uploadFileName);
            //  Edocs_Utilities.EdocsUtilitiesInstance.CHeckFileExists($"{batchDir}{uploadFileName}", false);
            byte[] pdfFile = UploadUtilities.ConvertToPdfFile($"{batchDir}{saveFileName}");
            saveFileName = $"{saveFileName.Replace(Path.GetExtension(saveFileName), ".pdf")}";
            string pdfSaveFName = $"{uploadFileName}{saveFileName}";
            File.WriteAllBytes(pdfSaveFName, pdfFile);

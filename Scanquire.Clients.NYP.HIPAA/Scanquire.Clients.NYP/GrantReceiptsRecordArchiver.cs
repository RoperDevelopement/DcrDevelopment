...
{
    public class GrantReceiptsRecordArchiver : AzureCloudArchiver
    {
        GrantReceiptsRecordDialog recordDialog = new GrantReceiptsRecordDialog();
        ...
        public override async Task Send(IEnumerable<SQImage> images, IProgress<EdocsUSA.Utilities.ProgressEventArgs> progress, System.Threading.CancellationToken cToken)
        {
            ...
            EDL.TraceLogger.TraceLoggerInstance.TraceInformation($"Starting Grant Receipts Record archiver batchid:{CurrentBatchId.ToString()} batchtime:{batchTime.ToString()}");
        ...
        {
            EDL.TraceLogger.TraceLoggerInstance.TraceInformation($"Sending file for Grant Receipts Record archiver batchid:{CurrentBatchId.ToString()}");
            
            string fileName = Guid.NewGuid().ToString();

            if (EnableAzureBatch)
            {
                EDL.TraceLogger.TraceLoggerInstance.TraceInformation($"Creating Azure batch for Grant Receipts Record archiver batchid:{CurrentBatchId.ToString()}");
                
                Dictionary<string, string> spRec = new Dictionary<string, string>();
                spRec[JsonsFieldConstants.JsonFieldDocumentDate] = recordDialog.DocumentDate.Value.Date.ToUniversalTime().ToString(SHAREPOINT_DATETIME_FORMAT);
                spRec[JsonsFieldConstants.JsonFieldClientCode] = recordDialog.ClientCode;
                spRec[JsonsFieldConstants.JsonFieldComments] = recordDialog.Comments;

...
namespace Scanquire.Public
{
	...
	public class SerializedObjectDictionaryCollection<T> where T : class
    {
		...
		public bool Remove(string key)
        {
            ...
            {
                EDL.TraceLogger.TraceLoggerInstance.TraceWarning("Error deleting directory " + directoryPath);
		...
		public void Refresh()
        {
            ...
            foreach (string directory in Directory.GetDirectories(DirectoryPath))
            {
                ...
                EDL.TraceLogger.TraceLoggerInstance.TraceInformation("Processing " + directory);
                string key = GetKeyFromDirectory(directory);
                SerializedObjectDictionary<T> value;
                if (TryDeserialize(directory, out value) == false)
                {
                    EDL.TraceLogger.TraceLoggerInstance.TraceInformation("Ignoring " + directory);
                }
                else
                {
                    Items[key] = value;
        ...
        protected bool TryDeserialize(string directory, out SerializedObjectDictionary<T> value)
        {
            try
            {
                ...
                {
                    EDL.TraceLogger.TraceLoggerInstance.TraceError("Directory not found " + directory);
            ...
            {
                EDL.TraceLogger.TraceLoggerInstance.TraceWarning("Error deserializing " + directory);
                EDL.TraceLogger.TraceLoggerInstance.TraceError(ex.Message);
                EDL.TraceLogger.TraceLoggerInstance.TraceError(ex.StackTrace);
                if (ex.InnerException != null)
                    EDL.TraceLogger.TraceLoggerInstance.TraceError($"Inner exc eption {ex.InnerException.Message}");

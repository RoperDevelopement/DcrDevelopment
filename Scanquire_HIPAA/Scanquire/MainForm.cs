...
using ScanQuireCropResizeImage;
using Edocs.Ocr.Pdf;
using FreeImageAPI;
    ...
    public partial class MainForm : Form
    {
        ...
        protected async Task SaveSelected(ISQArchiver archiver)
        {
            ...
            try
            {
                ...
                if (EDL.TraceLogger.TraceLoggerInstance.TotalImages == 0)
                {
                    MessageBox.Show(this, "No Images selected");
                }
                else
                {
        ...
        protected async Task Save(ISQArchiver archiver)
        {
            ...
            try
            {
                ...
                EDL.TraceLogger.TraceLoggerInstance.TraceInformation($"Done sending to archiver:{archiver.ToString()}");
                ImageListViewer.Saved = true;
                //foreach(SQImage image in ImageListViewer.CheckedImages.ToArray())
                //{
                //    SQImageListViewer listViewer = image.WorkingCopy.;
                //    ImageListViewer.Remove()
                //}
                //if (ClearOnSave)
                //{ ImageListViewer.Remove((false, true); }
                ProgressMonitor.StopMonitoring(progress, Scanquire.Public.UserControls.ProgressMonitor.StopMonitoringReason.ProcessCompleted);
            }
            catch (OperationCanceledException oe)
            {
        ...
        private void tsBtnEditImage_Click(object sender, EventArgs e)
        {
        ...
        {
            //if (ImageListViewer.UnChecked.Count() > 0)
            try
            {
                //CancellationTokenSource cTokenSource = new CancellationTokenSource();
                //CancellationToken cToken = cTokenSource.Token;
                ...
                //   ProgressMonitor.StartMonitoring(progress, cTokenSource); ProgressMonitor.StopMonitoring(progress, Scanquire.Public.UserControls.ProgressMonitor.StopMonitoringReason.ProcessFailed);
                SQImageListViewerItem item = ImageListViewer.ActiveItem;
                EditImages(item).ConfigureAwait(true).GetAwaiter().GetResult();
                //    ProgressMonitor.StopMonitoring(progress, Scanquire.Public.UserControls.ProgressMonitor.StopMonitoringReason.EditingImages);
            }
            catch (Exception ex)
            {
        ...
        private async void selectedToArchiveToolStripMenuItem_Click(object sender, EventArgs e)
        {
            CheckInactiveTimeOut();
            EDL.TraceLogger.TraceLoggerInstance.TraceInformation($"Start saving to archive folder: {CurrentArchiverSelector.SelectedArchiverName}");
            DialogResult result = MessageBox.Show($"Do You want to save to archiver {CurrentArchiverSelector.SelectedArchiverName}?", "Save Archiver", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
        ...
        private async void selectedToLocalFileToolStripMenuItem_Click(object sender, EventArgs e)
        {
            CheckInactiveTimeOut();
            EDL.TraceLogger.TraceLoggerInstance.TraceInformation($"Start saving to archive folder: {CurrentArchiverSelector.SelectedArchiverName}");
            DialogResult result = MessageBox.Show($"Do You want to save to archiver {CurrentArchiverSelector.SelectedArchiverName}?", "Save Archiver", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);

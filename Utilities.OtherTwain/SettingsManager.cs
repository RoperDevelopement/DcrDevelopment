...
namespace EdocsUSA.Utilities
{
	...
	public static class SettingsManager
	{
		...
		public static string TwainSettingsXmlFolder
		{
			get
			{
				{
					string configFile = Path.Combine(Edocs_Utilities.EdocsUtilitiesInstance.GetApplicationDir(), "EdocsUSA.Utilities.dll.config");
					string ts = Edocs_Utilities.EdocsUtilitiesInstance.GetAppConfigSetting("TwainSettingsXmlFolder", configFile);
					//if (!(string.IsNullOrWhiteSpace(scanQuireImages)))
					//	return Path.Combine($"{scanQuireImages}", $"{RelativePath}\\{DateTime.Now.ToString("MM_dd_yyyy")}");
					//// return Path.Combine(Path.GetTempPath(), RelativePath); 
					return Path.Combine($"{Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData)}{ts}");
				}
			}
		}
		public static string TwainSettingsCacheFolder
		{
			get
			{
				{
					string configFile = Path.Combine(Edocs_Utilities.EdocsUtilitiesInstance.GetApplicationDir(), "EdocsUSA.Utilities.dll.config");
					string ts = Edocs_Utilities.EdocsUtilitiesInstance.GetAppConfigSetting("TwainSettingsCacheFolder", configFile);
					//if (!(string.IsNullOrWhiteSpace(scanQuireImages)))
					//	return Path.Combine($"{scanQuireImages}", $"{RelativePath}\\{DateTime.Now.ToString("MM_dd_yyyy")}");
					//// return Path.Combine(Path.GetTempPath(), RelativePath); 
					return Path.Combine($"{Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData)}{ts}");
				...
				public static string CloseProcess
		{
			get
			{
				{
					string configFile = Path.Combine(Edocs_Utilities.EdocsUtilitiesInstance.GetApplicationDir(), "EdocsUSA.Utilities.dll.config");
					return Edocs_Utilities.EdocsUtilitiesInstance.GetAppConfigSetting("CloseProcess", configFile);

				}
			}
		}
		public static bool UpDatedCustomDataFile
		{ get; set; }
		public static string TwainSettingsUseXmlFile
		{
			get
			{
				{
					string configFile = Path.Combine(Edocs_Utilities.EdocsUtilitiesInstance.GetApplicationDir(), "EdocsUSA.Utilities.dll.config");
					return Edocs_Utilities.EdocsUtilitiesInstance.GetAppConfigSetting("UseXmlFile", configFile);

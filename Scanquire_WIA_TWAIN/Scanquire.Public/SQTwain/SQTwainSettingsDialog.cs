...
namespace Scanquire.Public
{
	...
	public class SQTwainSettingsDialog : TableLayoutPanelInputDialog
	{
		...
		protected void AddNewSetting()
		{
			...
			{
				customDSData = SQTwainSettings.Instance[SelectedScanner][SelectedSetting].CustomDSData;
			}
			if (SQTwain.Instance.UseTwain)
			{ 
			...
			else
            {
				...
				//EdocsUSA.Utilities.EdocsWia.WIA_Settings_Dialog settings_Dialog = new EdocsUSA.Utilities.EdocsWia.WIA_Settings_Dialog();
				
				using(var settings_Dialog = new EdocsUSA.Utilities.EdocsWia.WIA_Settings_Dialog())
				{
					if (customDSData != null)
						settings_Dialog.WIACustomaData = SQTwainSettings.Instance[SelectedScanner][SelectedSetting].CustomDSData;
					if(!(string.IsNullOrWhiteSpace(SelectedScanner)))
					          settings_Dialog.ActiveDataSource = SelectedScanner;
					settings_Dialog.ShowDialog();
				customDSData = settings_Dialog.WIACustomaData;
				if(customDSData == null)
							return;
				}

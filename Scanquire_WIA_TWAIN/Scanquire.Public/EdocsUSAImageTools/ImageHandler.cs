...
namespace Scanquire.Public.EdocsUSAImageTools
{
    public class ImageHandler
    {
        ...
        public async Task SaveBitmap(string saveFilePath)
        {
        ...
        public async Task RestorePrevious()
        {
            _currentBitmap =_bitmapbeforeProcessing;
        }

        public async Task SetColorFilter(ColorFilterTypes colorFilterType)
        {
        ...
        public async Task SetGamma(double red, double green, double blue)
        {
            Bitmap temp = (Bitmap)_currentBitmap;
            Bitmap bmap = (Bitmap)temp.Clone();
            Color c;
            byte[] redGamma = CreateGammaArray(red).ConfigureAwait(false).GetAwaiter().GetResult();
            byte[] greenGamma = CreateGammaArray(green).ConfigureAwait(false).GetAwaiter().GetResult();
            byte[] blueGamma = CreateGammaArray(blue).ConfigureAwait(false).GetAwaiter().GetResult();
            for (int i = 0; i < bmap.Width; i++)
            {
                for (int j = 0; j < bmap.Height; j++)
                {
        ...
        private async Task<byte[]> CreateGammaArray(double color)
        {
        ...
        public async Task SetContrast(double contrast)
        {
            Bitmap temp = (Bitmap)_currentBitmap;
            Bitmap bmap = (Bitmap)temp.Clone();
        ...
        public async Task SetGrayscale()
        {
            Bitmap temp = (Bitmap)_currentBitmap;
            Bitmap bmap = (Bitmap)temp.Clone();
            Color c;
        ...
        public async Task<System.Drawing.Image> ResizeImage(Image imgToResize, Size size)
        {
            return (System.Drawing.Image)(new Bitmap(imgToResize, size));
        }
        public async Task<System.Drawing.Bitmap> ResizeImage(Bitmap imgToResize, Size size)
        {
            return new Bitmap(imgToResize, size);
        }
        public async Task Resize(int newWidth, int newHeight)
        {
        ...
        public async Task Crop(int xPosition, int yPosition, int width, int height)
        {
            Bitmap temp = (Bitmap)_currentBitmap;
            Bitmap bmap = (Bitmap)temp.Clone();
        ...
        public async Task DrawOutCropArea(int xPosition, int yPosition, int width, int height)
        {
        ...
        public async Task RemoveCropAreaDraw()
        {
            _currentBitmap = (Bitmap)_bitmapPrevCropArea.Clone();
        }

        public async Task InsertText(string text, int xPosition, int yPosition, string fontName, float fontSize, string fontStyle, string colorName1, string colorName2)
        {
            Bitmap temp = (Bitmap)_currentBitmap;
            Bitmap bmap = (Bitmap)temp.Clone();
            Graphics gr = Graphics.FromImage(bmap);
        ...
        public async Task InsertImage(string imagePath, int xPosition, int yPosition)
        {
            Bitmap temp = (Bitmap)_currentBitmap;
            Bitmap bmap = (Bitmap)temp.Clone();
            Graphics gr = Graphics.FromImage(bmap);
        ...
        public async Task InsertImage(string imagePath, int xPosition, int yPosition,int w=0,int h=0)
        {
            Bitmap temp = (Bitmap)_currentBitmap;
            Bitmap bmap = (Bitmap)temp.Clone();
            Graphics gr = Graphics.FromImage(bmap);
            if (!string.IsNullOrEmpty(imagePath))
            {
                Bitmap i_bitmap = (Bitmap)Bitmap.FromFile(imagePath);
                if ((w > 0) || (h > 0))
                    i_bitmap = ResizeImage(i_bitmap, new Size(w, h)).ConfigureAwait(false).GetAwaiter().GetResult();
                Rectangle rect = new Rectangle(xPosition, yPosition, i_bitmap.Width, i_bitmap.Height);
                gr.DrawImage(Bitmap.FromFile(imagePath), rect);
            }
            _currentBitmap = (Bitmap)bmap.Clone();
        ...
        public async Task InsertShape(string shapeType, int xPosition, int yPosition, int width, int height, string colorName)
        {
            Bitmap temp = (Bitmap)_currentBitmap;
            Bitmap bmap = (Bitmap)temp.Clone();
            Graphics gr = Graphics.FromImage(bmap);
        ...
        public  double ConvertStrToPercent(string instr)
        {
            double retDouble = 0.0;
            int indexPer = instr.IndexOf('%');
            if(indexPer > 0)
            {
                string s = instr.Substring(0, indexPer);
                if (double.TryParse(instr.Substring(0,indexPer),out double results))
                {
                    return (results/100);
                }
            }

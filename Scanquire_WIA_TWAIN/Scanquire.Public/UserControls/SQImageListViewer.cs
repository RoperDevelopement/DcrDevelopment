...
namespace Scanquire.Public.UserControls
{
    ...
    public partial class SQImageListViewer : UserControl
    {
        ...
        private async void MoveImageUp_Click(object sender, EventArgs e)
        {
            await MoveUpImage(SelectedImages);
        }
        private async Task MoveUpImage(IEnumerable<SQImage> images)
        {
            int item = _Items.Count;
            ...
            for (int i = 0; i < _Items.Count; i++)
            {
                if (_Items[i].Selected)
                {
                    if(i==0)
                    {
                        MessageBox.Show($"Image at top", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        break;
                    }
                    int k = i;
                    SQImageListViewerItem sQImageListViewerItem = _Items[--k];
                    _Items[k] = _Items[i];
                    _Items[i] = sQImageListViewerItem;
                    break;
                }
        ...
        private async void MoveImageDown_Click(object sender, EventArgs e)
        {
            await MoveDownImage(SelectedImages);
        }
        private async Task MoveDownImage(IEnumerable<SQImage> images)
        {
            int item = _Items.Count;
            ...
            for (int i = 0; i < _Items.Count; i++)
            {
                ...
                if (_Items[i].Selected)
                {
                    if ((i +1)== _Items.Count)
                    {
                        MessageBox.Show($"Image at bottom", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        break;
                    }
                    int k = i+1;
                    SQImageListViewerItem sQImageListViewerItem = _Items[k];
        ...
        private void ThumbnailPanel_MouseHover(object sender, EventArgs e)
        {
          if(_Items.Count() > 0)
            {
                toolTip1.Active = true;
                toolTip1.SetToolTip(ThumbnailPanel, "Hold Shift to Select Multiple Images or Ctrl to Select One Image at a time");

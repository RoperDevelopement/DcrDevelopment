...
namespace Edocs.ITS.Archiver
{
    public class ITSArchiver : SQFilesystemArchiver
    {
        ...
        { get; set; }
        public string BSBSPUploadRecords
        { get; set; }
        ...
        public async override Task Send(SQFile file, int fileNumber, IProgress<EdocsUSA.Utilities.ProgressEventArgs> progress, System.Threading.CancellationToken cancelToken)
        {
            try
            {
                ...
                InputDialog.IncludeBlank = IncludeBlankDocs;
           ...
           InputDialog.CmbBoxItems = DropDownItems.Split(',');  
            ...
            settingsITS["AzureShareName"] = AzureShareName;
                ...
                settingsITS["AzureSPName"] = BSBSPUploadRecords;
            ...
            settingsITS["ITSAzureDataBaseName"] = ITSAzureDataBaseName;
                ...
                settingsITS["AddTitle"] = InputDialog.AddTitle.ToString();
            ...
            Log.Append(settingsITS);
                ...
                if (!(string.IsNullOrEmpty(InputDialog.ArchiveDate)))
                    UploadEntryITS["Date"] = InputDialog.ArchiveDate;
                else
                    UploadEntryITS["Date"] = "N/A";
                if (!(string.IsNullOrEmpty(InputDialog.Collection)))
                    UploadEntryITS["Collection"] = InputDialog.Collection;
                else
                    UploadEntryITS["Collection"] = "N/A";
                if (!(string.IsNullOrEmpty(InputDialog.Title)))
                    UploadEntryITS["Title"] = InputDialog.Title;
                else
                    UploadEntryITS["Title"] = "N/A";
                if (!(string.IsNullOrEmpty(InputDialog.Description)))
                    UploadEntryITS["Description"] = InputDialog.Description;
                else
                    UploadEntryITS["Description"] = "N/A";

...
/****** Object:  StoredProcedure [dbo].[uspPublishingVerification]    Script Date: 2/1/2016 4:50:55 PM ******/
SET ANSI_NULLS ON
GO
		...
		begin
			...
			where not exists(select [Topic ID] from [dbo].[PublishingVerification] pubver
			 ...
			 (case(@pubEnv) when 'mtps' then
				(select pubvermtps.[Topic ID] from [dbo].[PublishingVerification] pubvermtps where createXmldoc.TopicID = pubvermtps.[Topic ID] and pubvermtps.[Published Verification] = 'Created' and createXmldoc.DXVersionPublish = pubvermtps.[DX Version to Publish])
			  when 'soc' then
				(select pubversoc.[Topic ID] from [dbo].[PublishingVerification] pubversoc where createXmldoc.TopicID = pubversoc.[Topic ID] and pubversoc.[Create Publishing Date] = createXmldoc.CreatePublishingDate)

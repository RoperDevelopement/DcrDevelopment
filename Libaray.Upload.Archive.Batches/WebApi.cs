...
namespace Edocs.Libaray.Upload.Archive.Batches
{
    class WebApi
    {
        ...
        public static async Task UploadMDTTrackingInformation(string webAPiUri,string controllerName, object uploadObject)
        {
            ...
            using (var client = new HttpClient())
                ...
                JavaScriptSerializer serializer = new JavaScriptSerializer();
                          var ser = serializer.Serialize(uploadObject);
                           client.BaseAddress = new Uri(webAPiUri);
                ...
                else
                {
                    edl.TraceLogger.TraceLoggerInstance.TraceError($"Uplaoding json file {results.StatusCode.ToString()}");
                        edl.TraceLogger.TraceLoggerInstance.TraceError($"Uploading MDT files to webapi:{webAPiUri} controller:{controllerName}");
        ...
        public static async Task UploadBSBProdDepRec(string webAPiUri, string controllerName, object uploadObject)
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                string url = $"{client.BaseAddress.AbsoluteUri}{controllerName}";
                var responseTask = client.PostAsync($"{client.BaseAddress.AbsoluteUri}{controllerName}", content);
              //  var responseTask = client.GetAsync($"{client.BaseAddress.AbsoluteUri}{controllerName}");
                responseTask.Wait();
                ...
                else
                {
                    edl.TraceLogger.TraceLoggerInstance.TraceError($"Uplaoding json file {results.StatusCode.ToString()}");
                    edl.TraceLogger.TraceLoggerInstance.TraceError($"Uploading MDT files to webapi:{webAPiUri} controller:{controllerName}");
                    throw new Exception($"Uplaoding json file {results.StatusCode.ToString()} results {s.Result}");
        ...
        public static async Task UploadPSUSDRec(string webAPiUri, string controllerName, object uploadObject)
        {
        ...
        public static async Task UploadBSBProdSearchTxr(string webAPiUri, string controllerName, object uploadObject)
        {

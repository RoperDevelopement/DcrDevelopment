...
namespace Edocs.Pdf.Converter
{
    public class Pdf
    {
        ...
        public static void AddTextToPdf(string text,string fileName)
        {
            PDFLibrary pdf = new PDFLibrary();
            pdf.LoadFromFile(fileName,string.Empty);
           string s = pdf.GetPageText(0);
        ...
        public static async Task<string> GetImageOcrText(System.Drawing.Bitmap freeImage)
        {
            ...
            try
            {
                ...
                byte[] ocrImg = ImageToBase64(freeImage, System.Drawing.Imaging.ImageFormat.Jpeg);
                
                HttpClient httpClient = new HttpClient();
            httpClient.Timeout = new TimeSpan(1, 1, 1);
            MultipartFormDataContent form = new MultipartFormDataContent();
            form.Add(new StringContent("ca3780ed7488957"), "apikey"); //The "helloworld" key works, but it has a very low rate limit. You can get your won free api key at https://ocr.space/OCRAPI
            form.Add(new StringContent("eng"), "language");
            form.Add(new StringContent("1"), "OCREngine");
            form.Add(new StringContent("true"), "isTable");
            form.Add(new ByteArrayContent(ocrImg, 0, ocrImg.Length), "image", "image.jpg");
                HttpResponseMessage response = httpClient.PostAsync("https://api.ocr.space/Parse/Image", form).ConfigureAwait(true).GetAwaiter().GetResult(); ;
